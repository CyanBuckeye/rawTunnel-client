...
void pre_process_arg(int argc, char *argv[])
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        else
        {
            //printf("<%s>",argv[i]);
            new_argc++;
            new_argv.push_back(move(argv[i]));
        }
    }
    if(count>1)
    {
...
int start_tunnel(int argc, char *argv[])
{
    //printf("%s\n",my_ntoa(0x00ffffff));
    //auto a=string_to_vec("a b c d ");
    //printf("%d\n",(int)a.size());
    //pritf("%d %d %d %d",larger_than_u32(1,2),larger_than_u32(2,1),larger_than_u32(0xeeaaeebb,2),larger_than_u32(2,0xeeaaeebb));
    //assert(0==1);
...
Java_com_example_cppdemo_MainActivity_startService(JNIEnv *env, jobject instance, jstring str_) {
    /*
    char *args[4];
    args[0] = "/system/bin/iptables";
    args[1] = "-F";
    ...
    */
    ...
    char **argv = new char*[str_vec.size()];
    for(int i = 0; i < str_vec.size(); i++){
        argv[i] = new char[str_vec[i].length() + 1];
        int j = 0;
        for(j; j < str_vec[i].length(); j++)
            argv[i][j] = str_vec[i][j];
        argv[i][j] = '\0';
    }
    int argc = str_vec.size();
    start_tunnel(argc, argv);
    return;
}